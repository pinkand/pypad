...
// State for expanding/collapsing nodes (Expand all domains by default for rich graph visualization)
const expandedNodes = ref<Set<string>>(new Set([
  'python-core', 'env', 'syntax', 'control', 'ds', 'func', 'module', 'oop', 'file', 'robust'
]))
...
// Generate soft glowing blue/indigo circular texture for particles
...
const initParticleSystem = () => {
  ...
  // Apply initial particle style
  applyParticleStyle(appStore.bgAnimationStyle)
}

const applyParticleStyle = (style: 'cosmic' | 'slate') => {
  if (!particles) return
...
watch(() => appStore.bgAnimationStyle, (newStyle) => {
  applyParticleStyle(newStyle)
})
...
const animate = () => {
  ...
  if (particles && appStore.bgAnimationStyle === 'cosmic') {
...
const handleResize = () => {
  ...
  if (renderer) renderer.setSize(containerRef.value.clientWidth, containerRef.value.clientHeight)
  if (cssRenderer) cssRenderer.setSize(containerRef.value.clientWidth, containerRef.value.clientHeight)
}
...
onMounted(() => {
  try {
    ...
    console.warn('3D WebGL Canvas 初始化失败，已优雅防崩降级:', err)
  }
})
