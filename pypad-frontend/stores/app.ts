...
export const useAppStore = defineStore('app', () => {
  ...
  // Background Animation Style ('cosmic' | 'slate')
  const bgAnimationStyle = ref<'cosmic' | 'slate'>(
    (localStorage.getItem('pypad_bg_style') as 'cosmic' | 'slate') || 'cosmic'
  )

  const setBgAnimationStyle = (style: 'cosmic' | 'slate') => {
    bgAnimationStyle.value = style
    localStorage.setItem('pypad_bg_style', style)
  }
